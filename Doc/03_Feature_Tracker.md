...
| F-51 | AI Watchlist Alerts — Per-Stock Toggle | MVP | Pro+ | **Complete** | 100% | Full Stack | "Alerts" column shows price move pill (>3% flag) + analyst upgrade pill (if matching entry in `data.analyst`) per stock row. "AI" column shows per-row on/off toggle button. `aiOn` state as `Record<string, boolean>` (all true by default). `alerts(sym)` computes current alerts from mock data. `toggleAi(sym)` flips individual stock. File: `screens/watchlist.tsx`. |
